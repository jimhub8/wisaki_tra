...
if ( ! class_exists( 'YITH_Delivery_Date_Shipping_Manager' ) ) {
	...
	class YITH_Delivery_Date_Shipping_Manager {
		...
		public function print_delivery_from() {
			...
			if ( apply_filters( 'ywcdd_load_always_delivery_date', true, $chosen_methods ) ) {
		...
		public function get_format_available_date_range( $date_range ) {
			...
			foreach ( $date_range as $date ) {
				$available_days[] = date_i18n( $formats[ $current_format ], $date );
			}
		...
		public function format_timeslot( $available_timeslot ) {
			...
			if ( ! empty( $available_timeslot ) ) {
				foreach ( $available_timeslot as $slot_id => $slot ) {
					...
					$timeslotformat = apply_filters( 'yith_delivery_date_time_slot_format', $timeslotformat, $timefrom,$timeto, $is_taxable, $fee, $fee_name );
		...
		public function validate_checkout_width_delivery_date() {
			...
			if ( isset( $_POST['ywcdd_carrier'] ) && isset( $_POST['ywcdd_datepicker'] ) ) {
				...
				if( !empty( $option['select_process_method'] ) ){
		...
		public function add_delivery_date_info_order_meta( $order ) {
			...
			if ( isset( $_POST['ywcdd_datepicker'] ) ) {
				...
				$delivery_date = ywcdd_get_date_by_format( $delivery_date, 'Y-m-d' );
		...
		public function show_delivery_order_details( $order, $show_shipping = false ) {
			...
			$all_format     = yith_get_delivery_date_format();
			$current_format = get_option( 'yith_delivery_date_format', 'yy-mm-dd' );
			$date_format    = $all_format[ $current_format ];
			...
			if ( ! empty( $delivery_date ) ) {
				...
				$fields = array(
					...
					'shipping_date' => array(
						...
						'value' => sprintf( '%s %s', _x( 'within','[Part of]: Shipping Date within 20th March 2019', 'yith-woocommerce-delivery-date' ), ywcdd_get_date_by_format( $shipping_date, $date_format ) ),
					),
					'delivery_date' => array(
				...
				$fields = apply_filters( 'yith_delivery_date_email_fields', $fields, $show_shipping );
