...
if ( ! class_exists( 'YITH_Delivery_Date_Manager' ) ) {
	class YITH_Delivery_Date_Manager {
		...
		public function get_first_delivery_date( $id, $args = array() ) {
			...
			$min_workdays        = $args['min_working_day'];
			$end_by              = strtotime( ( $args['max_range'] + 1 ) . ' days', $today );
			$days_for_delivery   = ( ( $start_timestamp - $today ) / DAY_IN_SECONDS );
			...
			return $first_delivery_date ? $first_delivery_date : $end_by;
		}
		...
		public function get_all_delivery_dates( $id, $args = array() ) {
			...
			while ( $count_days < $args['max_range'] ) {
				...
				$day_have_slots = $this->get_available_time_slots( $id, $start_time_stamp );
			...
			$delivery_dates = apply_filters( 'ywcdd_add_custom_delivery_dates', array_unique( $delivery_dates ) );
		 ...
		 * @return int
		 */
		public function get_last_shipping_date( $date, $processing_id, $carrier_id ) {
			...
			$has_time_slot              = ! ( empty( $has_time_slot ) );
			$day_for_delivery           = 0;
			$end_by                     = strtotime( current_time( 'Y-m-d' ) );
		...
		public function get_available_time_slots( $carrier_id, $date, $check = false ) {
			...
			if ( count( $all_slots ) > 0 ) {
				...
				foreach ( $all_slots as $slot_id => $slot ) {
					...
					$check_time = $check_time || apply_filters( 'ywcdd_is_invalid_time_slot', false, $available_slots, $slot_id, $time_to, $time_from, $now_time, $date );


					$check_override_day = ( 'yes' === $slot['override_days'] && count( $slot['day_selected'] )> 0 && ! in_array( $wday, $slot['day_selected'] ) );
		...
		public function check_if_time_slot_is_lockout( $slot, $carrier_id, $date_selected ) {
			...
			if ( ( $slot['max_order'] != '' && $slot['max_order'] > 0 ) ) {
				$res     = $this->count_timeslot_order_used( $date_selected, $carrier_id );
				$key     = $slot['timefrom'] . '-' . $slot['timeto'];
				$key     = strtolower( $key );
				$max_ord = $slot['max_order'];

				if ( isset( $res[ $carrier_id ][ $key ] ) && ( $max_ord - $res[ $carrier_id ][ $key ] <= 0 ) ) {
					$is_lockout = true;
				}
			}
