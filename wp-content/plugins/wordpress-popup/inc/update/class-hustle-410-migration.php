	...
	public function restore( $check_if_exists = true ) {
		...
		try {
			...
			if ( $check_if_exists && ! $this->is_backup_created() ) {
				throw new Exception( __( "There's no backup to restore.", 'wordpress-popup' ) );
			}
			...
			if ( false === $deleted ) {
				throw new Exception( __( "The 4.1.x visibility metas couldn't be deleted.", 'wordpress-popup' ) );
			}
			...
			if ( false === $renamed ) {
				throw new Exception( __( "The 4.0.x visibility metas couldn't be restored.", 'wordpress-popup' ) );
			}
		} catch ( Exception $e ) {
