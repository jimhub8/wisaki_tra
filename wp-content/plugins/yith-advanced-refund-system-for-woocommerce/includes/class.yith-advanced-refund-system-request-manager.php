    ...
    class YITH_Advanced_Refund_System_Request_Manager {
		    ...
		    <form id="ywcars_form" method="post" enctype="multipart/form-data">
			    <h2 class="ywcars_order_title"><?php
				    if ( $is_whole_order ) {
					    ...
					    echo apply_filters( 'ywcars_refund_for_text', sprintf( __( 'Refund for %s', 'yith-advanced-refund-system-for-woocommerce' ), $product_name ), $product_id, $product_name, $order_id );
				    }
				    ?></h2>
			    ...
			    <div class="ywcars_block">
				    <label for="ywcars_form_reason"><?php
                        echo apply_filters( 'ywcars_reason_block_text', __( 'Please, write the reason why you are asking for a refund:',
	    ...
	    public function submit_request() {
		    ...
		    try {
			    ...
			    wc_add_notice( sprintf( apply_filters( 'ywcars_request_submitted_text', __( 'Request #%s submitted successfully', 'yith-advanced-refund-system-for-woocommerce' ), $request->ID ), $request->ID ) );
