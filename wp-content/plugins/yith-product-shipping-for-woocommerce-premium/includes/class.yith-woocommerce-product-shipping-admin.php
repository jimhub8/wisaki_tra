	...
	class YITH_WooCommerce_Product_Shipping_Admin {
		...
		public function add_yit_plugin_menu_item(){
			...
			if ( empty( $GLOBALS['admin_page_hooks']['yith_plugin_panel'] ) ) {
				...
				add_menu_page( 'yith_plugin_panel', 'YITH Plugins', 'edit_products', 'yith_plugin_panel', NULL, YIT_CORE_PLUGIN_URL . '/assets/images/yithemes-icon.png', $position );
			}
			...
			if ( $user_role == 'vendor' ) {
				...
				add_submenu_page(
	                null,
	                __( 'Vendor Shippings Table', 'yith-woocommerce-product-add-ons' ),
	                __( 'Vendor Shippings Table', 'yith-woocommerce-product-add-ons' ),
	                'edit_products',
	                'shipping-vendor',
		...
		function product_shipping_tab( $product_data_tabs ) {
			$product_data_tabs['yith-woocommerce-product-shipping'] = array(
				...
				'class'		=>  array( 'yith_wcps_tab_class' ),
			);
		...
		public function shipping_table( $post_id = 0 ) {
			...
			$show_table_price	= is_array( $table_cols ) && in_array( 'price', $table_cols ) && ! $is_edit_product && ! $not_editable;
					...
					<table class="widefat">
						...
						<tbody>
							<?php
								...
								$total_rows = $wpdb->get_row( $query_total, ARRAY_A )['COUNT(*)'];
								$results = $wpdb->get_results( $query, ARRAY_A );

								echo '<input type="hidden" name="pagination_offset" value="' . $pagination_offset . '"';
								// echo '<tr><td colspan="10">Offset = ' . $pagination_offset . '</td></tr>';
								...
								foreach ( $results as $result ) : ?>
									<tr>
										...
										<!-- CART TOTAL PRICE -->
										<td class="cart_total min"<?php echo $hide_table_price; ?>><input type="text" value="<?php echo esc_attr( ! $result['min_cart_total'] > 0 ? '' : strip_tags( wc_price( $result['min_cart_total'], array( 'currency' => 'false' ) ) ) ); ?>" placeholder="0.00" name="yith_product_min_cart_total[<?php echo $result['id'] > 0 ? $result['id'] : 'new]['; ?>]" <?php disabled( $not_editable ); ?> /></td>
										<td class="cart_total max"<?php echo $hide_table_price; ?>><input type="text" value="<?php echo esc_attr( ! $result['max_cart_total'] > 0 || $result['max_cart_total'] == '0.00' ? '' : strip_tags( wc_price( $result['max_cart_total'], array( 'currency' => 'false' ) ) ) ); ?>" placeholder="&infin;" name="yith_product_max_cart_total[<?php echo $result['id'] > 0 ? $result['id'] : 'new]['; ?>]" <?php disabled( $not_editable ); ?> /></td>
		...
		public function update_shippings( $post_id = 0 ) {
			...
			if ( $post_id > 0 ) {
				...
				update_post_meta( $post_id, '_yith_product_shipping_message', $_POST['_yith_product_shipping_message'] );
				update_post_meta( $post_id, '_yith_product_shipping_message_position', $_POST['_yith_product_shipping_message_position'] );
			}
			...
			if ( $min_quantities ) {
				...
				foreach ( $min_quantities as $key => $value ) {
					...
					if ( $key == 'new' ) {
						...
						$min_cart_total	= ! empty( $min_cart_totals[ $key ] )	? $min_cart_totals[ $key ]	: '';
						$max_cart_total	= ! empty( $max_cart_totals[ $key ] )	? $max_cart_totals[ $key ]	: '';
