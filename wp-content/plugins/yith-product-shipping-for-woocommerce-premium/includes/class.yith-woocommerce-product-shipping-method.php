					...
					if ( $values['quantity'] > 0 ) {
						if ( $values['data']->needs_shipping() ) {
							...
							if ( $shipping_row ) {
								...
								/*
								if ( $cart_unique_cost == 0 && $shipping_row->unique_cost > 0 && $single_unique_cost ) {
									$per_order_costs[ $shipping_row->ord ] = $shipping_row->unique_cost;
									$item_shipping_cost += $shipping_row->unique_cost;
									$single_unique_cost = false;
								}
								*/
			...
			if ( get_option( 'woocommerce_calc_taxes' ) == 'yes' && $this->tax_status == 'taxable' ) {
				...
				foreach ( array_keys( $taxes + $item_taxes ) as $key ) {
					...
					if ( get_option( 'woocommerce_tax_display_cart' ) == 'incl' ) {
						...
						$rate = $item_shipping_cost > 0 ? ( ( 100 / $item_shipping_cost ) * $current_tax ) : 0; // %

						$new_item_shipping_cost = $item_shipping_cost / ( 1 + ( $rate / 100 ) );
						$new_item_shipping_tax = $item_shipping_cost - $new_item_shipping_cost;
